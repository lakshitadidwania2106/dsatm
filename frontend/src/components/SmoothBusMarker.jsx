import { Marker, Popup } from 'react-leaflet'
import L from 'leaflet'
import { useEffect, useRef } from 'react'
import { useI18n } from '../hooks/useI18n'

// Bus icon SVG content
const busIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
  <path d="M5 11a1 1 0 1 1-2 0 1 1 0 0 1 2 0m8 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0m-6-1a1 1 0 1 0 0 2h2a1 1 0 1 0 0-2zm1-6c-1.876 0-3.426.109-4.552.226A.5.5 0 0 0 3 4.723v3.554a.5.5 0 0 0 .448.497C4.574 8.891 6.124 9 8 9s3.426-.109 4.552-.226A.5.5 0 0 0 13 8.277V4.723a.5.5 0 0 0-.448-.497A44 44 0 0 0 8 4m0-1c-1.837 0-3.353.107-4.448.22a.5.5 0 1 1-.104-.994A44 44 0 0 1 8 2c1.876 0 3.426.109 4.552.226a.5.5 0 1 1-.104.994A43 43 0 0 0 8 3"/>
  <path d="M15 8a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1V2.64c0-1.188-.845-2.232-2.064-2.372A44 44 0 0 0 8 0C5.9 0 4.208.136 3.064.268 1.845.408 1 1.452 1 2.64V4a1 1 0 0 0-1 1v2a1 1 0 0 0 1 1v3.5c0 .818.393 1.544 1 2v2a.5.5 0 0 0 .5.5h2a.5.5 0 0 0 .5-.5V14h6v1.5a.5.5 0 0 0 .5.5h2a.5.5 0 0 0 .5-.5v-2c.607-.456 1-1.182 1-2zM8 1c2.056 0 3.71.134 4.822.261.676.078 1.178.66 1.178 1.379v8.86a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 2 11.5V2.64c0-.72.502-1.301 1.178-1.379A43 43 0 0 1 8 1"/>
</svg>`

const createBusIcon = (isSelected, isVirtual) => {
    const bgColor = isVirtual
        ? '#eab308'
        : isSelected
            ? '#16a34a'
            : '#0f172a'

    const borderColor = isVirtual ? '#fef08a' : 'white'
    const iconSize = isVirtual ? 32 : 28
    const svgSize = iconSize - 8

    return L.divIcon({
        className: 'custom-bus-icon',
        html: `
      <div style="
        background-color: ${bgColor};
        width: ${iconSize}px;
        height: ${iconSize}px;
        border-radius: 50%;
        border: 3px solid ${borderColor};
        box-shadow: 0 2px 8px rgba(0,0,0,0.3);
        transition: all 0.3s ease;
        display: flex;
        align-items: center;
        justify-content: center;
        position: relative;
      ">
        <div style="
          width: ${svgSize}px;
          height: ${svgSize}px;
          display: flex;
          align-items: center;
          justify-content: center;
          color: white;
        ">
          ${busIconSvg.replace('width="16" height="16"', `width="${svgSize}" height="${svgSize}"`)}
        </div>
        ${isVirtual ? '<span style="position: absolute; top: -4px; right: -4px; font-size: 12px; color: #eab308;">★</span>' : ''}
      </div>
    `,
        iconSize: [iconSize, iconSize],
        iconAnchor: [iconSize / 2, iconSize / 2],
    })
}

export const SmoothBusMarker = ({ bus, isSelected, onSelect }) => {
    const markerRef = useRef(null)
    const { t } = useI18n()
    const isVirtual = bus.type === 'virtual'

    useEffect(() => {
        const marker = markerRef.current
        if (marker) {
            marker.setLatLng([bus.lat, bus.lng])
        }
    }, [bus.lat, bus.lng])

    return (
        <Marker
            ref={markerRef}
            position={[bus.lat, bus.lng]}
            icon={createBusIcon(isSelected, isVirtual)}
            eventHandlers={{ click: () => onSelect(bus) }}
        >
            <Popup>
                <strong>{bus.route}</strong>

                {isVirtual && (
                    <div className="text-xs text-yellow-600 font-bold mt-1">
                        Verified by {bus.passenger_count} Users
                    </div>
                )}

                <br />

                {!isVirtual && (
                    <>
                        {t('eta')}: {bus.eta}
                        <br />
                        {t('cost')}: {bus.cost}
                    </>
                )}
            </Popup>
        </Marker>
    )
}
